...
internal class DayTen {
	...
	private static (int x, int y) StartingPoint(List<List<char>> map) {
		...
		throw new Exception("No \'S\' found in map");
	}

	private static long MapToDistance(List<List<char>> map, (int x, int y) position) {
		long res = 0;
		(int x, int y) currentPosition = position;
		(int x, int y) previousPosition = position;
		char current;
		
		do { 
			(previousPosition, currentPosition) = (currentPosition, GetNextPosition(map, currentPosition, previousPosition));
			current = map[currentPosition.y][currentPosition.x];
			res++;
		} while(current != 'S');
	...
	private static (int, int) GetNextPosition(List<List<char>> map, (int x, int y) position, (int, int) previousPosition) {
		(int x, int y) res = position;
		char current = map[position.y][position.x];
		char left = position.x == 0 ? 'a' : map[position.y][position.x - 1];
		char up = position.y == 0 ? 'a' : map[position.y - 1][position.x];
		char right = position.x == map[0].Count - 1 ? 'a' : map[position.y][position.x + 1];
		char down = position.y == map.Count - 1 ? 'a' : map[position.y + 1][position.x];

		if((res.x - 1, res.y) != previousPosition && _left.Contains(left) && (current == 'S' || _right.Contains(current))) {
			res.x--;
		} else if((res.x, res.y - 1) != previousPosition && _up.Contains(up) && (current == 'S' || _down.Contains(current))) {
			res.y--;
		} else if((res.x +1, res.y) != previousPosition && _right.Contains(right) && (current == 'S' || _left.Contains(current))) {
			res.x++;
		} else if((res.x, res.y + 1) != previousPosition && _down.Contains(down) && (current == 'S' || _up.Contains(current))) {
			res.y++;
		}

		if(res == position) {
			throw new Exception("No new position found");
		}
	...
	private static long PartTwo(string data) {
		List<List<char>> map = InputTo2D(data);
		List<(int, int)> path = MapToPath(map, StartingPoint(map));
		Console.WriteLine(path.Count);

		var x = path.Select(i => i.Item1);
		var y = path.Select(i => i.Item2);

		Console.WriteLine(MapToContent(map, path, x.Min(), x.Max(), y.Min(), y.Max()));

		return path.Count % (MapToContent(map, path, x.Min(), x.Max(), y.Min(), y.Max()) + 1);
	}
	...
	private static List<(int, int)> MapToPath(List<List<char>> map, (int x, int y) position) {
		List<(int, int)> res = [];
		(int x, int y) currentPosition = position;
		(int x, int y) previousPosition = position;
		char current;
	...
	private static long MapToContent(List<List<char>> map, List<(int, int)> path, int x, int xMax, int y, int yMax) {
		long res = 0;
